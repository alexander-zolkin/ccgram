...
- Dead code: unused functions, constants, ProviderCapabilities fields, CLI options
- CCBOT\_\* legacy env var fallbacks (migration complete)
- Legacy session map prefix and hook marker from pre-rename ccbot
- EXPANDABLE*QUOTE*\* re-exports from providers layer (display-layer leak)
- Unused test fixtures and helpers

### Changed

- Updated .env.example with all supported env vars
...
- External-session/foreign-window adoption ([#105](https://github.com/alexei-led/ccgram/pull/105))
