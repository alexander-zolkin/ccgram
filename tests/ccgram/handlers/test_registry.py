...
def test_register_all_registers_all_handler_kinds():
    ...
    # 10 MessageHandlers: command trace (group=-1), FORUM_TOPIC_CLOSED,
    # FORUM_TOPIC_CREATED (fork: CCGRAM-HOTFIX:sticky-create-name),
    # FORUM_TOPIC_EDITED, COMMAND fallback, TEXT, PHOTO, Document.ALL, VOICE,
    # catch-all unsupported = 10
    assert by_kind.get(MessageHandler) == 10
