    ...
    *,
    ...
    with (
        ...
        registry.get.return_value.capabilities = MagicMock(
            ...
            # Fork's grok-initial-prompt / fresh-launch-args bakes the initial
            # prompt into the launch command when a provider accepts it; default
            # the fixture to the separate-forward path these tests exercise.
            launch_accepts_initial_prompt=False,
        )
        yield _LaunchMocks(
            mux=mux,
            session=session,
