...
class TestCrossScopeDedup:
    ...
    def test_repair_reports_repaired(self, router: ThreadRouter) -> None:
        # from_dict returns True on repair; the owning SessionManager persists.
        assert router.from_dict(self._split_state()) is True

    def test_clean_state_reports_no_repair(self, router: ThreadRouter) -> None:
        assert (
            router.from_dict(
                {
                    "thread_bindings": {},
